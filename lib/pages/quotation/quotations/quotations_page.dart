...
class QuotationsPage extends StatelessWidget {
  ...
  void showProductDetail(BuildContext context, QuotationModel quotation, QuotationsCtrl ctrl, int index){
    MyBottomSheet().show(context, Get.height/1.7, 
    ListView(
        children: <Widget>[
          Padding(
          ...
          Padding(
            ...
            child:Column(
              //crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                quotation.product == null ? SizedBox() :
                Column(
                  children: [
                    ListTile(
                      leading: Icon(LineIcons.tag, color: color500),
                      title: new Text(quotation.product!.name.toString(), style: body1),
                      subtitle:  new Text('producto', style: body2)
                    ),
                    ListTile(
                      leading: Icon(LineIcons.moneyBill, color: color500),
                      title: new Text(quotation.product!.price.toString(), style: body1),
                ...
                ListTile(
                  leading: Icon(LineIcons.moneyBill, color: color500),
                  title: new Text(quotation.subTotal.toString(), style: body1),
                  subtitle:  new Text('precio', style: body2)
                ),
                ListTile(
                  leading: Icon(LineIcons.moneyBill, color: color500),
                  title: new Text(quotation.quantity.toString() + " unds", style: body1),
                  subtitle:  new Text('cantidad', style: body2)
                ),
                ListTile(
                  leading: Icon(LineIcons.tag, color: color500),
                  title: new Text(DateFormat.yMMMMEEEEd('es_US').format(DateTime.fromMillisecondsSinceEpoch(quotation.expirationDate!)).toString(), style: body1),
          ...
          Text('gestionar cotización', style: subtitulo, textAlign: TextAlign.center,),
              SizedBox(height: 15,),
          
          quotation.status != 0 ?
          new Text(quotation.status == 1 ? 'La cotización fué aceptada' : 'La cotización fué rechazada'
          ,  style: TextStyle(
          ...
          new Row(
            ...
            children: [
              ...
              TextButton.icon(
                onPressed: ()=> ctrl.changeQuotationStatus(status: "quotesCanceled", quotation: quotation, index: index), 
                icon: Icon(LineIcons.thumbsDown, size: 20, color: Colors.redAccent), 
                label: Text('Rechazado',  style: body1)),
                TextButton.icon(
                onPressed: ()=>ctrl.changeQuotationStatus(status: "quotesSent", quotation: quotation, index: index), 
                icon: Icon(LineIcons.check, size: 20, color: Colors.greenAccent), 
                label: Text('Aceptado', style: body1)),
                TextButton.icon(
                onPressed: ()=> ctrl.shareQuotation(quotation: quotation), 
                icon: Icon(LineIcons.share, size: 20, color: Colors.blueAccent), 
                label: Text('compartir', style: body1,)),
              ],),
