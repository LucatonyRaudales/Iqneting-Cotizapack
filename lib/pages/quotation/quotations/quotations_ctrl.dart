...
class QuotationsCtrl extends GetxController {
  ...
  Future getQuotations(int? status) async {
    try {
      ...
      _quotationRepository.getQuotations(status).then((list) {
  ...
  void shareQuotation({required QuotationModel quotation}) {
    if (MyShareClass().canShare(expirationDate: quotation.expirationDate!)) {
      ...
      MyAlert.showMyDialog(
          ...
          message:
              'la cotización seleccionada expiró el día ${DateFormat.yMMMMEEEEd('es_US').format(DateTime.fromMillisecondsSinceEpoch(quotation.expirationDate!)).toString()}',
          color: Colors.grey);
    }
  }
  ...
  void changeQuotationStatus(
    ...
    QuotationRepository().updateQuotation(
        quotation: quotation, toUpdate: {"status": value}).then((val) async {
      Get.back();
      String toShow = value == 1 ? 'Aceptado' : 'Rechazado';
      MyAlert.showMyDialog(
          title: 'Estado actualizado',
          message:
              'se ha cambiado el estado de la cotización ${quotation.title} de generado a $toShow',
          color: Colors.green);
